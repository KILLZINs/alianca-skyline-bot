...
exports.default = {
    ...
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addSubcommandGroup(g => g.setName('servidor')
        ...
        .addSubcommandGroup(g => g.setName('manager')
    ...
    async execute(interaction) {
        ...
        const canManageGuilds = (0, allowlist_1.isBotManager)(interaction.user.id);
        const canManageManagers = (0, allowlist_1.isBotOwner)(interaction.user.id);
        if (!canManageGuilds) {
            return interaction.reply({
                embeds: [(0, embeds_1.errorEmbed)('Acesso Negado', 'Você não tem permissão para usar este comando.\nApenas donos e managers do bot podem usá-lo.')],
                ephemeral: true,
            });
        }
        if (group === 'manager' && !canManageManagers) {
            return interaction.reply({
                embeds: [(0, embeds_1.errorEmbed)('Acesso Negado', 'Apenas donos do bot podem gerenciar managers.')],
                ephemeral: true,
            });
        }
        ...
        if (group === 'servidor') {
            if (sub === 'adicionar') {
                ...
                return interaction.editReply({
                    embeds: [
                        (0, embeds_1.successEmbed)('Servidor Autorizado!', `**${guildName}** (\`${guildId}\`) foi adicionado à allowlist.\n\n` +
            ...
            if (sub === 'remover') {
                ...
                if (!existing) {
                    return interaction.editReply({ embeds: [(0, embeds_1.errorEmbed)('Não encontrado', `ID \`${guildId}\` não está na allowlist.`)] });
                }
                ...
                return interaction.editReply({
                    embeds: [
                        (0, embeds_1.successEmbed)('Servidor Removido', `**${existing.guildName ?? guildId}** (\`${guildId}\`) foi removido.\n` +
            ...
            if (sub === 'listar') {
                ...
                if (!guilds.length) {
                    return interaction.editReply({
                        embeds: [
                            (0, embeds_1.baseEmbed)(embeds_1.COLORS.WARNING)
            ...
            if (sub === 'status') {
                ...
                const embed = (0, embeds_1.baseEmbed)(active ? embeds_1.COLORS.SUCCESS : embeds_1.COLORS.WARNING)
                    ...
                    .addFields({ name: '📋 Modo', value: active ? '🔒 **Enforcement ativo**' : '🔓 **Bootstrap** (lista vazia — acesso livre)', inline: false }, { name: '✅ Servidores na lista', value: `**${count}**`, inline: true }, { name: '🤖 Bot está em', value: `**${botIn}** servidor(es)`, inline: true })
        ...
        if (group === 'manager') {
            if (sub === 'adicionar') {
                ...
                if ((0, allowlist_1.isBotOwner)(userId)) {
                    return interaction.editReply({ embeds: [(0, embeds_1.errorEmbed)('Desnecessário', 'Este usuário já é dono do bot.')] });
                }
                ...
                return interaction.editReply({
                    embeds: [(0, embeds_1.successEmbed)('Manager Adicionado', `<@${userId}> agora é manager do bot e pode gerenciar a allowlist.`)],
                });
            }
            if (sub === 'remover') {
                const userId = interaction.options.getString('user_id', true).trim();
                ...
                if (!existing)
                    return interaction.editReply({ embeds: [(0, embeds_1.errorEmbed)('Não encontrado', `<@${userId}> não é manager.`)] });
                await client_1.prisma.botManager.delete({ where: { userId } });
                (0, allowlist_1.cacheRemoveManager)(userId);
                return interaction.editReply({ embeds: [(0, embeds_1.successEmbed)('Manager Removido', `<@${userId}> não é mais manager.`)] });
            }
            if (sub === 'listar') {
                ...
                if (process.env.OWNER_ID)
                    ownerIds.push(process.env.OWNER_ID);
                const ownerLines = [...new Set(ownerIds)].map(id => `👑 <@${id}> — **Dono** (via env)`).join('\n');
                const managerLines = managers.map(m => `🔧 <@${m.userId}>${m.username ? ` — ${m.username}` : ''}\n> Adicionado por <@${m.addedBy}> <t:${Math.floor(m.addedAt.getTime() / 1000)}:D>`).join('\n\n');
...
//# sourceMappingURL=botadmin.js.map