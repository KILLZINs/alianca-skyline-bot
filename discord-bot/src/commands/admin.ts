...
import { isAllianceServerRepresentative } from '../utils/alliance';
...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const isAdminUser  = await isAdmin(member, interaction.guild.id);
    const isServerOwner = interaction.guild.ownerId === interaction.user.id;
    const isRep        = await isAllianceServerRepresentative(interaction.guild.id, interaction.user.id);

    if (!isAdminUser && !isServerOwner && !isRep) {
      return interaction.reply({
        embeds: [errorEmbed('Sem Permissão', 'Apenas administradores, donos ou representantes da aliança podem usar este painel.')],
        ephemeral: true,
      });
    }
    ...
    // admin:stats removido — use /servidor → Estatísticas
